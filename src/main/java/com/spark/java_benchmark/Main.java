...
public class Main {
	...
	public static HashMap<String, Long> measureQueryExecutionTime(SparkSession spark, String parquetPath,
		...
		return new HashMap<>() {{
			put("execution_time", timeElapsed);
			put("records_count", cnt);
		}};
	}
		...
		for (int i = 0; i < TEST_REPEAT_COUNT; i++) {
			...
			for(int j = 0; j < spark_queries.size(); j++){
				...
				LOGGER.info(String.format("\t%d-st query: elapsed time %s ms, selected %s records", j + 1,
						withLargeIntegers(res.get("execution_time")), withLargeIntegers(res.get("records_count"))));
				testTimes[j] += res.get("execution_time");
			}
		}
		LOGGER.info("SUMMARY");
